...
const TOKEN_KEY = "ctrl-custo-token";
...
export function setToken(t: string) {
  _token = t;
  SecureStore.setItemAsync(TOKEN_KEY, t).catch(() => undefined);
}
...
export const api = {
  ...
  accounts: {
    ...
    create: (data: NewAccount) =>
      req<ApiAccount>("/accounts", { method: "POST", body: JSON.stringify(data) }).then(mapAccount),
  ...
  cards: {
    ...
    create: (data: NewCard) =>
      req<ApiCard>("/cards", { method: "POST", body: JSON.stringify(data) }).then(mapCard),
